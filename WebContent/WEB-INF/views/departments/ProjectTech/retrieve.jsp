...
<script type="text/javascript">
	...
	function retrieveFiles()
	{
		...
		if(Sector=='' && Category=='' && Name_Of_Work=='' && Contractor_Name=='' && Department=='' && FileNumber=='')
			setContent('Empty Parameters!');
		else
			document.getElementById('ProjectTechForm').submit();
	}
	function deleteFile()
	{
		window.location="delView?department=ProjectTech";
	}
	...
	{
		if(right==1)
			window.location="downloadFile?id="+opaFts+"&sno="+sno+"&department=ProjectTech";
	}
	function updateFile(sno,right)
	{
		if(right==1)
			window.location="updateFile?sno="+sno+"&department=ProjectTech";
	}
	...
	function printOut(opaFts,sno,right,contractorName)
	{
		if(right==1)
		{
			var url="viewFile?id="+opaFts+"&sno="+sno+"&department=ProjectTech&prFlage=print&name="+contractorName;
...
<c:if test="${not empty records}">
	<form action="generateReport" id="reportForm" method="post">
		<input type="hidden" name="department" value="ProjectTech">
			...
			<tbody>
				<c:forEach items="${records}" var="record">
					<tr>
						...
						<td><a href="#" onclick="updateFile('${record.sno}','${update}')" style="text-decoration: none;">${record.fileNumber}</a></td>
						<td>${record.sector}</td>
